...
$Audit = $Name
$Report = "WINAudit_Security_Configuration_Report.txt"
...
Note: This data should be used to correlate registry audit findings and can be located within 'RawData'.`n" >> $Destination\$Audit\$Report
...
# System Information
Write-Output "General Checks: System" >> $Destination\$Audit\$Report
try
{
...
Write-Output "General Checks: Network" >> $Destination\$Audit\$Report
try
{
...
# PowerShell Version
Write-Output "General Checks: PowerShell Version" >> $Destination\$Audit\$Report
try
{
...
# PowerShell States
Write-Output "General Checks: PowerShell States" >> $Destination\$Audit\$Report
try
{
...
# Hotfixes
Write-Output "General Checks: Hotfixes" >> $Destination\$Audit\$Report
try
{
...
# AntiVirus
Write-Output "General Checks: AntiVirus" >> $Destination\$Audit\$Report
try
{
...
# Installed Programs
Write-Output "General Checks: Installed Programs" >> $Destination\$Audit\$Report
try
{
...
# Scheduled Tasks
Write-Output "General Checks: Scheduled Tasks" >> $Destination\$Audit\$Report
try
{
...
# Local Users
Write-Output "General Checks: Local Users" >> $Destination\$Audit\$Report
try
{
...
Write-Output "General Checks: Local Administrators" >> $Destination\$Audit\$Report
try
{
...
Write-Output "General Checks: Internet Connection Test" >> $Destination\$Audit\$Report
...
{
	Write-Output "Information: Internet Connection Test not carried out, as system is not a Windows server." >> $Destination\$Audit\$Report
}

########## Configuration Auditing ##########
Write-Output "`n### Configuration Audit" >> $Destination\$Audit\$Report

# LAPS
Write-Output "`nCheck: Local Administrator Password Solution (LAPS)" >> $Destination\$Audit\$Report
$LAPSPath = "C:\Program Files\LAPS\CSE\AdmPwd.dll"
if (Test-Path $LAPSPath)
{
	Write-Output "Information: Local Administrator Password Solution (LAPS) is installed." >> $Destination\$Audit\$Report
}
	else
{
	Write-Output "Finding: Local Administrator Password Solution (LAPS) is not installed. `nBackground: Threat actors may attempt to perform Pass-the-Hash (PtH) techniques in order to carry out lateral movement against target hosts using the same local Administrator password hashes. `nRecommendation: Install Local Administrator Password Solution (LAPS) on all hosts, to ensure local Administrator passwords are rotated and managed centrally." >> $Destination\$Audit\$Report
}

# WDigest Protocol
Write-Output "`nCheck: WDigest Protocol" >> $Destination\$Audit\$Report
...
# Credential Guard
Write-Output "`nCheck: Credential Guard" >> $Destination\$Audit\$Report
...
Write-Output "`nCheck: Credential Guard Virtualization Security" >> $Destination\$Audit\$Report
...
Write-Output "`nCheck: Credential Guard Platform Security Level" >> $Destination\$Audit\$Report
...
Write-Output "`nCheck: Safe DLL Search Mode" >> $Destination\$Audit\$Report
...
Write-Output "`nCheck: Remote Host Credential Delegation" >> $Destination\$Audit\$Report
...
Write-Output "`nCheck: Remote SAM Enumeration" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $RemoteSAMEnumKey $RemoteSAMEnumValue)
{
	...
	if ($RemoteSAMEnumData -Eq "1") {
		Write-Output "Information: Remote SAM Enumeration '$RemoteSAMEnumValue' is set to '1'. Remote SAM Enumeration Restriction is enabled." >> $Destination\$Audit\$Report}
...
Write-Output "`nCheck: Anonymous Enumeration " >> $Destination\$Audit\$Report
...
Write-Output "`nCheck: Anonymous SAM Enumeration" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $RestrictAnonSAMKey $RestrictAnonSAMValue)
{
	...
	if ($RemoteSAMEnumData -Eq "1") {
		Write-Output "Information: Restrict Anonymous SAM Enumeration '$RestrictAnonSAMValue' is set to '1'. Restrict Anonymous SAM Enumeration is enabled." >> $Destination\$Audit\$Report}
	elseif ($RemoteSAMEnumData -Eq "0") { 
...
Write-Output "`nCheck: Link Local Multicast Name Resolution (LLMNR)" >> $Destination\$Audit\$Report
...
Write-Output "`nCheck: NetBIOS Name Service (NBT-NS)" >> $Destination\$Audit\$Report
...
Write-Output "`nCheck: Cached Domain Credentials" >> $Destination\$Audit\$Report
...
Write-Output "`nCheck: Cached Domain Credentials Count" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $DomainCredsCountKey $DomainCredsCountValue)
{
	...
	elseif ($DomainCredsCountData -Eq "0") { 
		Write-Output "Information: Cached Domain Credentials Count '$DomainCredsCountValue' is set to $DomainCredsCountData. Cached Domain Credentials (hashes) from previous interactive logons is disabled." >> $Destination\$Audit\$Report
}
}
    else
{
    Write-Output "Information: Cached Domain Credentials Count '$DomainCredsCountValue' does not exist, therefore Cached Domain Credentials (hashes) from previous interactive logons is disabled." >> $Destination\$Audit\$Report
}

# Non Admin Safe Mode Block
Write-Output "`nCheck: Non Admin Safe Mode Block" >> $Destination\$Audit\$Report
...
Write-Output "`nCheck: Command Line Process Auditing" >> $Destination\$Audit\$Report
...
# SMBv1
Write-Output "`nCheck: SMBv1" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $SMB1Key $SMB1Value)
{
	...
	if ($SMB1Data -Eq "1") {
		Write-Output "Finding: '$SMB1Value' is installed. '$SMB1Value' is enabled. `nBackground: Threat actors may attempt to abuse the highly vulnerable SMB version 1 service. `nRecommendation: Set registry key '$SMB1Key' value '$SMB1Value' to '$SMB1Recommended', to ensure SMB version 1 is disabled." >> $Destination\$Audit\$Report}
	elseif ($SMB1Data -Eq "0") { 
		Write-Output "Information: '$SMB1Value' is installed. '$SMB1Value' is disabled." >> $Destination\$Audit\$Report
}
}
    else
{
    Write-Output "Information: '$SMB1Value' does not exist, therefore '$SMB1Value' is enabled. `nBackground: Threat actors may attempt to abuse the highly vulnerable SMB version 1 service. `nRecommendation: Set registry key '$SMB1Key' value '$SMB1Value' to '$SMB1Recommended', to ensure SMB version 1 is disabled." >> $Destination\$Audit\$Report
}

# SMBv2
Write-Output "`nCheck: SMBv2" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $SMB2Key $SMB2Value)
{
	...
	if ($SMB2Data -Eq "1") {
		Write-Output "Information: '$SMB2Value' is installed. '$SMB2Value' is enabled." >> $Destination\$Audit\$Report}
	elseif ($SMB2Data -Eq "0") { 
		Write-Output "Information: '$SMB2Value' is installed. '$SMB2Value' is disabled." >> $Destination\$Audit\$Report
}
}
    else
{
    Write-Output "Information: '$SMB2Value' does not exist, therefore '$SMB2Value' is enabled." >> $Destination\$Audit\$Report
}

# SMB Require Security Signature Client
Write-Output "`nCheck: SMB Require Security Signature Client" >> $Destination\$Audit\$Report
...
Write-Output "`nCheck: SMB Require Security Signature Server" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $SMBRequireSSServerKey $SMBRequireSSServerValue)
{
	...
	if ($SMBRequireSSServerData -Eq "1") {
		Write-Output "Information: SMB Require Security Signature Server '$SMBRequireSSServerValue' is set to '1'. SMB packet signing requirement is enabled for servers - Microsoft network client: Digitally sign communications (always)." >> $Destination\$Audit\$Report}
...
Write-Output "`nCheck: SMB Enable Security Signature Client" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $SMBEnableSSClientKey $SMBEnableSSClientValue)
{
	...
	if ($SMBEnableSSClientData -Eq "1") {
		Write-Output "Information: SMB Enable Security Signature Client '$SMBEnableSSClientValue' is set to '1'. SMB packet signing enablement is enabled for clients - Microsoft network server: Digitally sign communications (if client agrees)." >> $Destination\$Audit\$Report}
...
Write-Output "`nCheck: SMB Enable Security Signature Server" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $SMBEnableSSServerKey $SMBEnableSSServerValue)
{
	...
	if ($SMBEnableSSServerData -Eq "1") {
		Write-Output "Information: SMB Enable Security Signature Server '$SMBEnableSSServerValue' is set to '1'. SMB packet signing enablement is enabled for servers - Microsoft network server: Digitally sign communications (if server agrees)." >> $Destination\$Audit\$Report}
...
Write-Output "`nCheck: Printer Spooler CVE-2021-34527 [PrintNightmare]" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $PrintNightmareKey $PrintNightmare2Value)
{
	...
	elseif ($PrintNightmare2Data -Eq "0") { 
		Write-Output "Information: Printer Spooler [PrintNightmare] '$PrintNightmare2Value' is set to '0'. Printer Spooler [PrintNightmare] is not vulnerable." >> $Destination\$Audit\$Report
}
}
    else
{
    Write-Output "Information: Printer Spooler [PrintNightmare] '$PrintNightmare2Value' does not exist, therefore Printer Spooler [PrintNightmare] is not vulnerable." >> $Destination\$Audit\$Report
}

# Printer Spooler CVE-2021-34481
Write-Output "`nCheck: Printer Spooler CVE-2021-34481" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $PrinterSpoolerRCEKey $PrinterSpoolerRCEValue)
{
	...
	if ($PrinterSpoolerRCEData -Eq "1") {
		Write-Output "Information: Printer Spooler Driver Restriction '$PrinterSpoolerRCEValue' is set to '1'. Printer Spooler Driver Restriction is enabled." >> $Destination\$Audit\$Report}
	elseif ($PrinterSpoolerRCEData -Eq "0") { 
		Write-Output "Finding: Printer Spooler Driver Restriction '$PrinterSpoolerRCEValue' is set to '0'. Printer Spooler Driver Restriction is disabled. `nBackground: Threat actors may use attempt to exploit CVE-2021-34481 a remote code execution (RCE) vulnerability in the Printer Spooler service. `nRecommendation: Set registry key '$PrinterSpoolerRCEKey' value '$PrinterSpoolerRCEValue' to '$PrinterSpoolerRCERecommended', to prevent non-administrator users from installing Point and Print drivers. Microsoft knowledge base KB5005652." >> $Destination\$Audit\$Report
}
}
    else
{
    Write-Output "Information: Printer Spooler Driver Restriction '$PrinterSpoolerRCEValue' does not exist, therefore Printer Spooler Driver Restriction is enabled." >> $Destination\$Audit\$Report
}

# Hijack Execution Flow Unquoted Path
Write-Output "`nCheck: Hijack Execution Flow Unquoted Path" >> $Destination\$Audit\$Report
...
{
    Write-Output "Information: Legacy PowerShell versions not installed." >> $Destination\$Audit\$Report
}

# PowerShell Module Logging
Write-Output "`nCheck: PowerShell Module Logging" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $PSModuleLogKey $PSModuleLogValue)
{
	...
	if ($PSModuleLogData -Eq "1") {
		Write-Output "Information: PowerShell Module Logging '$PSModuleLogValue' is set to '1'. PowerShell Module Logging is enabled." >> $Destination\$Audit\$Report
		# Fetch Module Names
	...
	elseif ($PSModuleLogData -Eq "0") { 
		Write-Output "Finding: PowerShell Module Logging '$PSModuleLogValue' is set to '0'. PowerShell Module Logging is disabled. `nBackground: PowerShell Module Logging auditing is not configured by default, therefore this limits visibility and detection opportunities. `nRecommendation: Set registry key '$PSModuleLogKey' value '$PSModuleLogValue' to '$PSModuleRecommended' and specify notable/abnormal modules to monitor. Monitor Windows EID 4103 for enhanced visibility." >> $Destination\$Audit\$Report
}
    else
{
...
Write-Output "`nCheck: PowerShell Script Block Logging" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $PSScriptBlockLogInvoKey $PSScriptBlockLogInvoValue)
{
	...
	if ($PSScriptBlockLogInvoData -Eq "1") {
		Write-Output "Finding: PowerShell Script Block Logging Invocation '$PSScriptBlockLogInvoValue' is set to '1'. PowerShell Script Block Logging Invocation is enabled. `nBackground: PowerShell Script Block Invocation can provide additional context, however is very noisy and is not required unless testing is carried out. `nRecommendation: Set registry key '$PSScriptBlockLogInvoKey' value '$PSScriptBlockLogInvoValue' to '$PSScriptBlockLogInvoRecommended', to ensure Script Block Invocation is disabled." >> $Destination\$Audit\$Report}
	elseif ($PSScriptBlockLogInvoData -Eq "0") { 
		Write-Output "Information: PowerShell Script Block Logging Invocation '$PSScriptBlockLogInvoValue' is set to '0'. PowerShell Script Block Logging Invocation is disabled." >> $Destination\$Audit\$Report
}
}
    else
{
    Write-Output "Information: PowerShell Script Block Logging Invocation '$PSScriptBlockLogInvoValue' does not exist, therefore PowerShell Script Block Logging Invocation is disabled." >> $Destination\$Audit\$Report
}

# PowerShell Transcription Logging
Write-Output "`nCheck: PowerShell Transcription Logging" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $PSTranscriptLogKey $PSTranscriptLogValue)
{
	...
	if ($PSTranscriptLogData -Eq "1") {
		Write-Output "Information: PowerShell Transcription Logging '$PSTranscriptLogValue' is set to '1'. PowerShell Transcription Logging is enabled." >> $Destination\$Audit\$Report
...
Write-Output "`nCheck: PowerShell Transcription Logging Invocation" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $RDPAuthKey $RDPAuthValue)
{
	...
	if ($RDPAuthData -Eq "1") {
		Write-Output "Information: Remote Desktop Authentication '$RDPAuthValue' is set to '1'. Remote Desktop Network-Level Authentication is enabled." >> $Destination\$Audit\$Report}
...
Write-Output "`nCheck: Remote Desktop Unsolicited" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $RDPUnsolicitedKey $RDPUnsolicitedValue)
{
	...
	elseif ($RDPUnsolicitedData -Eq "0") { 
		Write-Output "Information: Remote Desktop Unsolicited '$RDPUnsolicitedValue' is set to '0'. Remote Desktop Unsolicited requests is disabled." >> $Destination\$Audit\$Report
}
}
    else
{
    Write-Output "Information: Remote Desktop Unsolicited '$RDPUnsolicitedValue' does not exist, therefore Remote Desktop Unsolicited requests is disabled." >> $Destination\$Audit\$Report
}

# Remote Desktop Security Layer
Write-Output "`nCheck: Remote Desktop Security Layer" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $RDPSecurityLayerKey $RDPSecurityLayerValue)
{
	...
	elseif ($RDPSecurityLayerData -Eq "2") { 
		Write-Output "Information: Remote Desktop Security Layer '$RDPSecurityLayerValue' is set to '2'. Remote Desktop Security Layer is using SSL/TLS required." >> $Destination\$Audit\$Report
	}
}
}
    else
...
Write-Output "`nCheck: Remote Desktop Max Idle Time" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $RDPMaxIdleTimeKey $RDPMaxIdleTimeValue)
{
	...
	elseif ($RDPMaxIdleTimeData -Le $RDPMaxIdleTimeRecommended) { 
		Write-Output "Information: Remote Desktop Max Idle Time '$RDPMaxIdleTimeValue' is less than or equal to 15 minutes (RAW: $RDPMaxIdleTimeData). Remote Desktop Max Idle Time is sufficient." >> $Destination\$Audit\$Report
}
}
    else
{
...
Write-Output "`nCheck: Remote Desktop Max Disconnection Time" >> $Destination\$Audit\$Report
...
Write-Output "`nCheck: Windows Automatic Updates" >> $Destination\$Audit\$Report
...
Write-Output "`nCheck: User Account Controls (UAC) aka Limited User Account (LUA)" >> $Destination\$Audit\$Report
...
Write-Output "`nCheck: PowerShell Operational Winevtx Max Size" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $PowEvtxMaxSizeKey $PowEvtxMaxSizeValue)
{
	...
	if ($PowEvtxMaxSizeData -Ge "$PowEvtxMaxSizeRecommended") {
		Write-Output "Information: PowerShell Operational Windows Event Log Max Size '$PowEvtxMaxSizeValue' is greater than or equal to 100MB (RAW: $PowEvtxMaxSizeData). PowerShell Operational winevtx log retention is sufficient." >> $Destination\$Audit\$Report}
...
Write-Output "`nCheck: Security Winevtx Max Size" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $SecEvtxMaxSizeKey $SecEvtxMaxSizeValue)
{
	...
	if ($SecEvtxMaxSizeData -Ge "$SecEvtxMaxSizeRecommended") {
		Write-Output "Information: Security Windows Event Log Max Size '$SecEvtxMaxSizeValue' is greater than or equal to 1GB (RAW: $SecEvtxMaxSizeData). Security winevtx log retention is sufficient." >> $Destination\$Audit\$Report}
...
Write-Output "`nCheck: System Winevtx Max Size" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $SysEvtxMaxSizeKey $SysEvtxMaxSizeValue)
{
	...
	if ($SysEvtxMaxSizeData -Ge "$SysEvtxMaxSizeRecommended") {
		Write-Output "Information: System Windows Event Log Max Size '$SysEvtxMaxSizeValue' is greater than or equal to 100MB (RAW: $SysEvtxMaxSizeData). System winevtx log retention is sufficient." >> $Destination\$Audit\$Report}
...
Write-Output "`nCheck: Application Winevtx Max Size" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $AppEvtxMaxSizeKey $AppEvtxMaxSizeValue)
{
	...
	if ($AppEvtxMaxSizeData -Ge "$AppEvtxMaxSizeRecommended") {
		Write-Output "Information: Application Windows Event Log Max Size '$AppEvtxMaxSizeValue' is greater than or equal to 100MB (RAW: $AppEvtxMaxSizeData). Application winevtx log retention is sufficient." >> $Destination\$Audit\$Report}
...
Write-Output "`nCheck: Security Winevtx Access" >> $Destination\$Audit\$Report
...
Write-Output "`nCheck: System Winevtx Access" >> $Destination\$Audit\$Report
...
Write-Output "`nCheck: Application Winevtx Access" >> $Destination\$Audit\$Report
...
Write-Output "`nCheck: Windows Defender Disable AntiSpyware" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $DefAntiSpywareKey $DefAntiSpywareValue)
{
	...
	elseif ($DefAntiSpywareData -Eq "0") { 
		Write-Output "Information: Windows Defender AntiSpyware '$DefAntiSpywareValue' is set to '0'. AntiSpyware is enabled." >> $Destination\$Audit\$Report
}
}
    else
{
    Write-Output "Information: Windows Defender AntiSpyware '$DefAntiSpywareValue' does not exist, therefore AntiSpyware is enabled." >> $Destination\$Audit\$Report
}

# Windows Defender Service Startup
Write-Output "`nCheck: Windows Defender Service Startup" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $DefServiceStartupKey $DefServiceStartupValue)
{
	...
	if ($DefServiceStartupData -Eq "1") {
		Write-Output "Information: Windows Defender Fast Service Startup '$DefServiceStartupValue' is set to '1'. Fast Service Startup is enabled." >> $Destination\$Audit\$Report}
...
Write-Output "`nCheck: Windows Defender Service Alive" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $DefServiceAliveKey $DefServiceAliveValue)
{
	...
	if ($DefServiceAliveData -Eq "1") {
		Write-Output "Information: Windows Defender Service Keep Alive '$DefServiceAliveValue' is set to '1'. Service Keep Alive is enabled." >> $Destination\$Audit\$Report}
...
Write-Output "`nCheck: Windows Defender PUA" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $DefPUAKey $DefPUAValue)
{
	...
	if ($DefPUAData -Eq "1") {
		Write-Output "Information: Windows Defender Block Potentially Unwanted Applications '$DefPUAValue' is set to '1'. Blocking of PUA is enabled." >> $Destination\$Audit\$Report}
...
Write-Output "`nCheck: Windows Defender Routine Remediation" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $DefRoutineKey $DefRoutineValue)
{
	...
	elseif ($DefRoutineData -Eq "0") { 
		Write-Output "Information: Windows Defender Routine Remediation '$DefRoutineValue' is set to '0'. Automatic Remediation is enabled." >> $Destination\$Audit\$Report
}
}
    else
{
    Write-Output "Information: Windows Defender Routine Remediation '$DefRoutineValue' does not exist, therefore Automatic Remediation is enabled." >> $Destination\$Audit\$Report
}

# Windows Defender Realtime Monitoring
Write-Output "`nCheck: Windows Defender Realtime Monitoring" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $DefRealtimeKey $DefRealtimeValue)
{
	...
	elseif ($DefRealtimeData -Eq "0") { 
		Write-Output "Information: Windows Defender Realtime Monitoring '$DefRealtimeValue' is set to '0'. Realtime Monitoring is enabled." >> $Destination\$Audit\$Report
}
}
    else
{
    Write-Output "Finding: Windows Defender Realtime Monitoring '$DefRealtimeValue' does not exist, therefore Realtime Monitoring is enabled." >> $Destination\$Audit\$Report
}

# Windows Defender IOAV
Write-Output "`nCheck: Windows Defender IOAV" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $DefIOAVKey $DefIOAVValue)
{
	...
	elseif ($DefIOAVData -Eq "0") { 
		Write-Output "Information: Windows Defender IOAV Protection '$DefIOAVValue' is set to '0'. IOAV Protection is enabled." >> $Destination\$Audit\$Report
}
}
    else
{
    Write-Output "Finding: Windows Defender IOAV Protection '$DefIOAVValue' does not exist, therefore IOAV Protection is enabled." >> $Destination\$Audit\$Report
}

# Windows Defender Behaviour Monitoring
Write-Output "`nCheck: Windows Defender Behaviour Monitoring" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $DefBehaviourKey $DefBehaviourValue)
{
	...
	elseif ($DefBehaviourData -Eq "0") { 
		Write-Output "Information: Windows Defender Behaviour Monitoring '$DefBehaviourValue' is set to '0'. Behaviour Monitoring is enabled." >> $Destination\$Audit\$Report
}
}
    else
{
    Write-Output "Finding: Windows Defender Behaviour Monitoring '$DefBehaviourValue' does not exist, therefore Behaviour Monitoring is enabled." >> $Destination\$Audit\$Report
}

# Windows Defender Removable Drive Scanning
Write-Output "`nCheck: Windows Defender Removable Drive Scanning" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $DefRemovableKey $DefRemovableValue)
{
	...
	elseif ($DefRemovableData -Eq "0") { 
		Write-Output "Information: Windows Defender Removable Drive Scanning '$DefRemovableValue' is set to '0'. Removable Drive Scanning is enabled." >> $Destination\$Audit\$Report
}
}
    else
{
...
Write-Output "`nCheck: Windows Defender Email Scanning" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $DefEmailKey $DefEmailValue)
{
	...
	elseif ($DefEmailData -Eq "0") { 
		Write-Output "Information: Windows Defender Email Scanning '$DefEmailValue' is set to '0'. Email Scanning is enabled." >> $Destination\$Audit\$Report
}
}
    else
{
...
Write-Output "`nCheck: Windows Defender Block At First Sight (MAPS)" >> $Destination\$Audit\$Report
...
if (Test-RegistryValue $DefBAFSKey $DefBAFSValue)
{
	...
	elseif ($DefBAFSData -Eq "0") { 
		Write-Output "Information: Windows Defender Block At First Sight '$DefBAFSValue' is set to '0'. Block At First Sight is enabled." >> $Destination\$Audit\$Report
}
}
    else
{
    Write-Output "Finding: Windows Defender Block At First Sight '$DefBAFSValue' does not exist, therefore Block At First Sight is enabled." >> $Destination\$Audit\$Report
}

# Windows Defender Notification
Write-Output "`nCheck: Windows Defender Notification" >> $Destination\$Audit\$Report
...
{
    Write-Output "Information: Windows Defender Notification Suppress '$DefNotificationValue' does not exist, therefore Notifications are enabled." >> $Destination\$Audit\$Report
}

# Do Not Run Microsoft Office Checks On Servers
Write-Output "`nCheck: Microsoft Office Checks" >> $Destination\$Audit\$Report
if($OS[0] -notlike "*Server*") {
	...
	Write-Output "`nCheck: Excel Dynamic Data Exchange (DDE) Protocol Launch" >> $Destination\$Audit\$Report
	...
	Write-Output "`nCheck: Excel Dynamic Data Exchange (DDE) Protocol Server Lookup" >> $Destination\$Audit\$Report
	...
	if (Test-RegistryValue $ExcelDDESrvLookupKey $ExcelDDESrvLookupValue)
	{
		...
		if ($ExcelDDESrvLookupData -Eq "0") {
			Write-Output "Information: Excel Dynamic Data Exchange (DDE) Protocol Server Lookup '$ExcelDDESrvLookupValue' is set to '0'. Excel Dynamic Data Exchange (DDE) Protocol Server Lookup is disabled." >> $Destination\$Audit\$Report}
	...
	Write-Output "`nCheck: Word Dynamic Data Exchange (DDE) Protocol" >> $Destination\$Audit\$Report
	...
	Write-Output "`nCheck: Excel (Office 2007>) Link Warnings - Dynamic Data Exchange (DDE)" >> $Destination\$Audit\$Report
	...
	if (Test-RegistryValue $ExcelLinkWarnKey $ExcelLinkWarnValue)
	{
		...
		if ($ExcelLinkWarnData -Eq "2") {
			Write-Output "Information: Automatic Update of Workbook Links '$ExcelLinkWarnValue' is set to '2'. Automatic Update of Workbook Links is disabled." >> $Destination\$Audit\$Report}
	...
	Write-Output "`nCheck: Word (Office 2010>) Link Warnings - Dynamic Data Exchange (DDE)" >> $Destination\$Audit\$Report
	...
	if (Test-RegistryValue $WordLinkWarnKey $WordLinkWarnValue)
	{
		...
		if ($WordLinkWarnData -Eq "1") {
			Write-Output "Information: Automatic Update of Word Links '$WordLinkWarnValue' is set to '1'. Automatic Update of Word Links is disabled." >> $Destination\$Audit\$Report}
	...
	Write-Output "`nCheck: Legacy Word (Office 2007) Link Warnings - Dynamic Data Exchange (DDE)" >> $Destination\$Audit\$Report
	...
	if (Test-RegistryValue $LegacyWordLinkWarnKey $LegacyWordLinkWarnValue)
	{
		...
		if ($LegacyWordLinkWarnData -Eq "1") {
			Write-Output "Information: Automatic Update of Word Links '$LegacyWordLinkWarnValue' is set to '1'. Automatic Update of Word Links is disabled." >> $Destination\$Audit\$Report}
	...
	Write-Output "`nCheck: Outlook (Office 2010>) Link Warnings - Dynamic Data Exchange (DDE)" >> $Destination\$Audit\$Report
	...
	if (Test-RegistryValue $OutlookLinkWarnKey $OutlookLinkWarnValue)
	{
		...
		if ($OutlookLinkWarnData -Eq "1") {
			Write-Output "Information: Automatic Update of Outlook Links '$OutlookLinkWarnValue' is set to '1'. Automatic Update of Outlook Links is disabled." >> $Destination\$Audit\$Report}
	...
	Write-Output "`nCheck: Legacy Outlook (Office 2010>) Link Warnings - Dynamic Data Exchange (DDE)" >> $Destination\$Audit\$Report
	...
	if (Test-RegistryValue $LegacyOutlookLinkWarnKey $LegacyOutlookLinkWarnValue)
	{
		...
		if ($LegacyOutlookLinkWarnData -Eq "1") {
			Write-Output "Information: Automatic Update of Outlook Links '$LegacyOutlookLinkWarnValue' is set to '1'. Automatic Update of Outlook Links is disabled." >> $Destination\$Audit\$Report}
	...
	# Excel VBA Warnings
	Write-Output "`nCheck: Excel VBA Warnings" >> $Destination\$Audit\$Report
	...
	Write-Output "`nCheck: Word VBA Macro Warnings" >> $Destination\$Audit\$Report
	...
	if (Test-RegistryValue $WordVBAWarnKey $WordVBAWarnValue)
	{
		...
		if ($WordVBAWarnData -Eq "2") {
			Write-Output "Information: Word VBA Warnings '$WordVBAWarnValue' is set to '2'. VBA macros are fully disabled (with notification)." >> $Destination\$Audit\$Report}
		elseif ($WordVBAWarnData -Eq "1") {
			Write-Output "Finding: Word VBA Warnings '$WordVBAWarnValue' is set to '1'. VBA macros are fully enabled. `nBackground: Threat actors may exploit macrosfor malicious execution. `nRecommendation: Set registry key '$WordVBAWarnKey' value '$WordVBAWarnValue' to '$WordVBAWarnRecommended', to ensure Word VBA macros are not exploited for malicious execution." >> $Destination\$Audit\$Report
	...
	Write-Output "`nCheck: Excel Mark-Of-The-Web Macro Block" >> $Destination\$Audit\$Report
	...
	Write-Output "`nCheck: Word Mark-Of-The-Web Macro Block" >> $Destination\$Audit\$Report
	...
	Write-Output "`nCheck: Excel Internet Files Protected View" >> $Destination\$Audit\$Report
	...
	Write-Output "`nCheck: Word Internet Files Protected View" >> $Destination\$Audit\$Report
	...
	if (Test-RegistryValue $WordInternetPVKey $WordInternetPVValue)
	{
		...
		if ($WordInternetPVData -Eq "0") {
			Write-Output "Information: Word Internet Files Protected View '$WordInternetPVValue' is set to '0'. Word Internet Files Protected View is enabled." >> $Destination\$Audit\$Report}
	...
	Write-Output "`nCheck: Excel Attachment Files Protected View" >> $Destination\$Audit\$Report
	...
	if (Test-RegistryValue $ExcelAttachmentPVKey $ExcelAttachmentPVValue)
	{
		...
		if ($ExcelAttachmentPVData -Eq "0") {
			Write-Output "Information: Excel Attachment Files Protected View '$ExcelAttachmentPVValue' is set to '0'. Excel Attachment Files Protected View is enabled." >> $Destination\$Audit\$Report}
	...
	Write-Output "`nCheck: Word Attachment Files Protected View" >> $Destination\$Audit\$Report
	...
	if (Test-RegistryValue $WordAttachmentPVKey $WordAttachmentPVValue)
	{
		...
		if ($WordAttachmentPVData -Eq "0") {
			Write-Output "Information: Word Attachment Files Protected View '$WordAttachmentPVValue' is set to '0'. Word Attachment Files Protected View is enabled." >> $Destination\$Audit\$Report}
	...
	Write-Output "`nCheck: Excel Unsafe Location Protected View" >> $Destination\$Audit\$Report
	...
	Write-Output "`nCheck: Word Unsafe Location Protected View" >> $Destination\$Audit\$Report
...
{
	Write-Output "Information: Microsoft Office Checks not carried out, as system is a Windows server." >> $Destination\$Audit\$Report
}
